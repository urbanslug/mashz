...
segtable* new_segment_table
   (u32			size,
	...
	st->seg           = st->_seg;  // (hook up the segment table)
...
//----------
//
// subset_segment_table--
//	Create a subset of a segment table, sharing the actual elements in the
//	table.
//
// NOTE: This just fills the subset object with information that fools it into
//       thinking it has its own seg[] array, but it actually points into the
//       other obejct's seg[] array.  Tread lightly!
//
//----------
//
// Arguments:
//	segtable*	st:			The segment table to "copy".
//	u32			startIx:	First entry in st->seg that should be included in
//							the subset.
//	u32			endIx:		One past the last entry in st->seg that should be
//							included in the subset.
//	segtable*	subset:		The segment table object to write the subset to.
//							Note that we don NOT expect this to have been
//							allocated from the head, and in any event
//							free_segment_table() should NOT be used to dispose
//							of it.
//
// Returns:
//	(nothing)
...
void subset_segment_table
   (segtable*	st,
	u32			startIx,
	u32			endIx,
	segtable*	subset)
	{
	subset->size          = endIx - startIx;
	subset->len           = subset->size;
	subset->haveScores    = st->haveScores;
	subset->coverageLimit = st->coverageLimit;
	subset->coverage      = st->coverage;
	subset->lowScore      = st->lowScore;
	subset->seg           = &st->seg[startIx];
	}
...
segtable* read_segment_table
   (FILE*			f,
	...
	unspos			tOffset, tLen, qOffset, qLen, qTrue, qNegStart;
	char*			queryName   = "";
	char			queryStrand = '+';
...
//	pos1, pos2, length, s, id, hspId:	The segment to add.  (Positions are
//										.. origin-zero).
//
// Returns:
...
//
// Returns:
//	nothing;  the segment table is modified in place, with segments NOT marked
//	for filtering brought to the front, and the table shortened by modifying
//	st->len.
//
//----------
...
void filter_marked_segments
   (segtable*	st)
	...
	if (st      == NULL) return;
//	if (st->seg == NULL) return;  clang flags this;  seg is an array, not a
//	                              pointer, so it can never be NULL;  unless the
//	                              implementation of segments is changed, which
//	                              is why I build in this sanity check
...
//	int			subsample:  <=0 => write all segments
//							>0  => write every Nth segment
//
// Returns:
//	(nothing)
//
...
void write_segments
   (FILE*			f,
	...
	for (segIx=0,seg=st->seg ; segIx<st->len ; segIx++,seg++)
		{
		if ((subsample != 0) && (segIx % subsample != 0)) continue;
