...
std::string create(const std::string& base,
    ...
    if (handler.parent_directory.empty()) {
        ...
        string tmpdirname_cpp = get_dir(); // + "/mashz-XXXXXX";
        ...
        if (got != nullptr) {
            ...
            cerr << "[mashz]: couldn't create temp directory: " << tmpdirname << endl;
            exit(1);
        }
        delete [] tmpdirname;
    }
    */
    ...
    if(fd != -1) {
        ...
        cerr << "[mashz]: couldn't create temp file on base "
             << base << " : " << tmpname << endl;
        exit(1);
    }
